...
const DashAffecterEncadrent = () => {
  ...
  const getProjet = async () => {
    try {
      const response = await axiosInstance.get(`/projet/en-attenteR`);
  ...
  const changeEtatConfirmer = async (idProjet) => {
    try {
      const response = await axiosInstance.patch(
        `/projet/change-etat/confirmer/${idProjet}`
      );
      console.log(response.data);
      getProjet();
    } catch (error) {
            ...
            {projects.map((project) => (
              <tr key={project._id}>
                <td className="flex">
                  ...
                  <button
                    onClick={() => {
                      changeEtatConfirmer(project._id);
                    }}
...
export default DashAffecterEncadrent;
