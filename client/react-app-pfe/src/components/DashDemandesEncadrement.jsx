...
const DashDemandesEncadrement = () => {
  ...
  const getProjet = async () => {
    try {
      const response = await axiosInstance.get(`/projet/en-attente`);
  ...
  const getEtudiant = async (idEtudiant) => {
    try {
      const response = await axiosInstance.get(`/utilisateur/user/${idEtudiant}`);
      console.log(response.data);
  ...
  const changeEtatEnAttenteR = async (idProjet) => {
    try {
      const response = await axiosInstance.patch(
        `/projet/change-etat/enAttenteR/${idProjet}`
      );
      console.log(response.data);
      getProjet();
    } catch (error) {
  ...
  const changeEtatAnnuler = async (idProjet) => {
    try {
      const response = await axiosInstance.patch(
        `/projet/change-etat/annuler/${idProjet}`
      );
      console.log(response.data);
      getProjet();
    } catch (error) {
            ...
            {projects.map((project) => (
              <tr key={project._id}>
                <td className="flex">
                  ...
                  <button
                    onClick={() => {
                      changeEtatEnAttenteR(project._id);
                    }}
